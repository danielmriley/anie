...
| 03 — Web SSRF and redirect boundary | ✅ | PR A `ea5dd9f` (manual redirect validation). PR B `40ef55a` (DNS resolver abstraction + resolved-IP guard, IPv4-mapped IPv6, Class E reserved). PR C landed (conservative headless gate: pre-launch DNS validation, runtime warning, doc + tool-description SSRF caveat). Chrome request interception deferred. |
