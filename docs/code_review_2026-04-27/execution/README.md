...
| 04 — Web cancellation, budgets, and bounded side channels | ✅ | PR A `8b09c8f` (cancellation). PR B `a34ec65` (bounded side channels). PR C `8fbba97` (`[tools.web]` config + validation). PR D landed (web_read emits coarse fetching/rendering/extracting phase pings via existing partial-update plumbing; best-effort try_send so a slow consumer can't backpressure the fetch). |
