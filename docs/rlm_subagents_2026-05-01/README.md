...
| 5 | [05_parallel_decomposition.md](05_parallel_decomposition.md) | **REVISED:** parallel execution of independent sub-tasks from PR 4's plan (was originally voting + critic; rejected as wasteful). **Shipped:** dry-run round annotation (PR 5) + concurrent executor (PR 5.1). Behind `ANIE_PARALLEL_DECOMPOSE>=2`. **Provider-aware concurrency:** API providers honor the env value (capped at 6); Ollama clamps to N=1 sequential by default — see [`concurrency_decisions.md`](concurrency_decisions.md). `ANIE_PARALLEL_DECOMPOSE_FORCE=1` overrides the Ollama clamp for users with capable hardware. |
