...
| 4 | [04_failed_tool_result_eviction.md](04_failed_tool_result_eviction.md) | "Context rot" from failed tool results that linger after they're no longer relevant (Cursor harness post-mortem 2026-05-02) | **shipped (Signal A + Signal B)**: failed tool results whose `(tool_name, args_hash)` match a later successful call evict ahead of FIFO order (Signal A); failures positioned in the leading portion of working that have aged past the still-fresh window evict next (Signal B, position-based heuristic). Embedding-based low-relevance ranking (the original plan's design for Signal B) deferred to v2 if the position-based heuristic proves insufficient. |
