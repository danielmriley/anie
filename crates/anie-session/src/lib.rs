...
/// |         | compaction.ts:~33`.                                   |
/// | 5       | Two independent additions merged under one bump:      |
/// |         | (a) `SessionEntry::BranchSummary` variant — a record  |
...
/// |         | (b) `SessionHeader.name` optional user-set display    |
/// |         | name for the session, surfaced in `/session list` so  |
