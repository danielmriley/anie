...
//!
//! Tracks the future settings UI from `docs/ideas.md`.
...
const BODY: &str =
    "Settings overlay not yet implemented. Edit `~/.anie/config.toml` directly for now.";
...
impl OverlayScreen for SettingsScreen {
...
mod tests {
    ...
    #[test]
    fn settings_placeholder_renders_title_and_body() {
    ...
    #[test]
    fn settings_placeholder_dismisses_on_any_key() {
        let mut screen = SettingsScreen::new();
        for key in [
    ...
    #[test]
    fn settings_placeholder_tick_keeps_open() {
        let mut screen = SettingsScreen::new();
