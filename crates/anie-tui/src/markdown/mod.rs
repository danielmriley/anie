...
//!
//! Entry point: [`render_markdown`]. `OutputPane` calls this on
//! both finalized and in-flight assistant content; the streaming
//! path renders the full accumulated text every frame
//! (`tui_polish_2026-04-26/PR1`) and relies on the block-level
//! cache to keep the cost bounded.
//!
//! Caching lives at the `OutputPane` block-cache layer (PR 2 of
