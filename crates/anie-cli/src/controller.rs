...
const RLM_SYSTEM_PROMPT_AUGMENT: &str = "\n\n# Context virtualization (rlm mode)\n\nThis run uses an external archive of prior conversation. Every tool call you issue is recorded there with its arguments. Each turn you receive a `<system-reminder>` ledger listing the URLs, queries, commands, and paths already used. Each entry includes its real `[id=...]` so you can address it directly.\n\nWhen the user asks a follow-up question, FIRST scan the ledger. If the answer would come from a URL, query, command, or path already listed, do NOT re-run the tool. Use `recurse` instead:\n  - `scope.kind=message_grep`, `pattern=<regex>` — search archived messages by keyword. Easiest option; needs no id. Use this first when you don't have a specific id in hand.\n  - `scope.kind=tool_result`, `tool_call_id=<id>` — fetch one prior result verbatim. Use the exact `[id=...]` value from the ledger; do NOT invent ids.\n  - `scope.kind=summary`, `id=<archive_id>` — fetch the gist (cheapest).\n\nThis applies even to live-world questions (weather, news, prices) when the relevant pages are already in the archive — re-fetching wastes the user's time. Reach for `web_read` / `web_search` only when no archived material would answer the question.";
