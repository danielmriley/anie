...
fn builtin_commands() -> Vec<SlashCommandInfo> {
    vec![
        ...
        SlashCommandInfo::builtin_with_args(
            "checkpoint",
            "Record a working-tree checkpoint you can rewind to",
            ArgumentSpec::FreeForm { required: false },
            Some("[name]"),
        ),
        SlashCommandInfo::builtin_with_args(
            "rewind",
            "List rewind points, or restore the tree + conversation to one",
            ArgumentSpec::FreeForm { required: false },
            Some("[<entry-id>]"),
        ),
        SlashCommandInfo::builtin_with_args(
            "loop",
            "Re-submit a prompt on an interval (stop with `/loop stop`)",
            ArgumentSpec::FreeForm { required: false },
            Some("[<Nm|Ns> <message> | stop]"),
        ),
        SlashCommandInfo::builtin_with_args(
            "goal",
            "Work autonomously toward a goal until done (stop with `/goal stop`)",
            ArgumentSpec::FreeForm { required: false },
