...
/// - 2026-05-02: Old prompt said "independently-solvable"
///   which pushed the model to flatten real dependencies.
///   Now invites `(depends on N)` markers explicitly.
/// - 2026-05-02: Old NO_PLAN_NEEDED guidance was vague.
///   Now names concrete cases (single-fact, single-computation).
/// - 2026-05-03: Long-running smoke showed plans overshooting
///   the user's immediate request — Run B's T1 ("write a DLL
///   class") produced a plan that included writing the
///   driver + compiling + testing, packing T1+T2+T3's work
///   into T1 and running 45 min vs. Run A's 5.5 min for the
///   same prompt. New prompt explicitly constrains the plan
///   to the user's stated scope.
const DECOMPOSE_SYSTEM_PROMPT: &str = "You are a planning assistant. Given a user's task, break it into 3-7 focused sub-tasks. Output as a numbered list, one sub-task per line, no other prose. Each sub-task should be small enough that a sub-agent could complete it in one focused pass.\n\nSCOPE: Plan ONLY the user's stated request. Do NOT extend the plan to natural follow-ups (testing, packaging, deploying) unless the user asked for them. If the user said \"write a function\", plan the function — not the tests or the driver. If they want more, they'll ask in a follow-up turn.\n\nDEPENDENCIES: If a sub-task depends on another, mark the dependency by appending `(depends on N)` to the sub-task line, where N is the prior sub-task number. Use `(depends on 2, 3)` for multiple. Sub-tasks WITHOUT a marker are assumed independent — and may be executed concurrently. Mark dependencies accurately: don't pretend everything is independent when it isn't, and don't invent dependencies where none exist.\n\nIf the user's task is a single-fact lookup, a single computation (e.g. `what is 7 * 8?`), or otherwise too trivial to benefit from a plan, output the single line: NO_PLAN_NEEDED. Don't produce a 1-sub-task plan — that adds noise without value.";
