...
struct SkillFrontmatter {
    ...
    /// Suggested tools, surfaced when the skill is staged via
    /// `/skill:<name>`. Parsed and displayed, not yet enforced
    /// at tool-call time (see `docs/skills_loader/README.md` §8).
...
impl SkillRegistry {
    ...
    /// Test-only direct insert, for sibling modules that need a
    /// registry with known contents without touching the
    /// filesystem. Production code should use [`Self::discover`].
    #[cfg(test)]
    pub(crate) fn insert_for_test(&mut self, skill: Skill) {
        self.insert_with_precedence(skill);
    }
